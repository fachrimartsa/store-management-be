...
const barangService = {
  ...
  updateBarang: async ({ brg_id, brg_nama, brg_kategori, brg_harga_beli, brg_harga_jual, brg_stok, brg_status }) => {
    try {
      // kita buat dinamically update query pake string
      const updates = [];
      const values = [];
      
      if (brg_nama !== undefined) {
        updates.push(`brg_nama = ${brg_nama}`);
      }
      if (brg_kategori !== undefined) {
        updates.push(`brg_kategori = ${brg_kategori}`);
      }
      if (brg_harga_beli !== undefined) {
        updates.push(`brg_harga_beli = ${brg_harga_beli}`);
      }
      if (brg_harga_jual !== undefined) {
        updates.push(`brg_harga_jual = ${brg_harga_jual}`);
      }
      if (brg_stok !== undefined) {
        updates.push(`brg_stok = ${brg_stok}`);
      }
      if (brg_status !== undefined) {
        updates.push(`brg_status = ${brg_status}`);
      }
      ...
      const query = `
        UPDATE barang SET ${updates.join(", ")} 
        WHERE brg_id = ${brg_id}
        RETURNING brg_id, brg_nama, brg_kategori, brg_harga_beli, brg_harga_jual, brg_stok, brg_status
      `;

      const result = await db.unsafe(query); // db.unsafe buat raw string query
